...
import { TableSpotPanel } from './TableSpotPanel'
...
export function EquityPage() {
  const t = useT()
  // Two ways in. "Spot" starts from the table and derives the pot; "Ranges"
  // is the original form, where the pot is typed in and nothing is assumed
  // about how it got there.
  const [inputMode, setInputMode] = useState<'spot' | 'ranges'>('spot')
  ...
  const runEquity = () => {
    ...
    setTimeout(() => {
      try {
        ...
        // The calling grid only means anything heads-up against a range: a
        // single known hand is one matchup rather than a range to read, and
        // multiway the price is not a single opponent's to set.
        const lone = villains.length === 1 && villains[0].mode === 'range' ? villains[0] : null
        setGrid(lone ? handEquities(lone.range) : null)
      } catch (err) {
        setResult(null)
        setGrid(null)
    ...
    <div className="min-h-screen p-4 sm:p-6 max-w-6xl mx-auto">
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        {(['spot', 'ranges'] as const).map((mode) => (
          ...
          >
            {mode === 'spot' ? t('A table') : t('Ranges only')}
          </button>
        ))}
      </div>
      ...
      <section className="mt-4 bg-slate-900/60 rounded-lg border border-slate-800 p-4 space-y-4">
        ...
        {result && (
          <div className="mt-4 space-y-3">
            ...
            {grid && result.callEv?.requiredEquityPct !== undefined && (
              <CallingRangeSection
                grid={grid}
                thresholdPct={result.callEv.requiredEquityPct}
                noBountyThresholdPct={result.callEv.requiredEquityNoBountyPct}
              />
            )}
